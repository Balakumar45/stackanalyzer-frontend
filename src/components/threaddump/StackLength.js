...
const StackLengthStats = ({ uploadDate, threadDumpFiles }) => {
  ...
  useEffect(() => {
    if (uploadDate && threadDumpFiles) {
      ...
      const url = `${apiBaseUrl}/api/threaddump/get-stack-length-stats?${queryString}`;
  ...
  const fetchStackTraces = (category) => {
    ...
    const queryString = `uploadDate=${uploadDate}&threadDumpFile=${threadDumpFiles}&category=${category}`;
    const url = `${apiBaseUrl}/api/threaddump/get-stacks-by-length?${queryString}`;
