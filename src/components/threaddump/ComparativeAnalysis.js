...
const ComparativeAnalysis = ({ uploadDate, threadDumpFiles }) => {
  ...
  useEffect(() => {
    if (uploadDate && threadDumpFiles?.length > 0) {
      ...
      const queryString = `uploadDate=${uploadDate}&` + threadDumpFiles.map(file => `threadDumpFiles=${file}`).join('&');
  ...
  useEffect(() => {
    if (uploadDate && 
      threadDumpFiles?.length > 0 &&
      threadSummary && 
      !loadingSummary) {
      setLoadingStates(true);
      const queryString = `uploadDate=${uploadDate}&` + threadDumpFiles.map(file => `threadDumpFile=${file}`).join('&');
      const url = `${apiBaseUrl}/api/threaddump/get-comparative-thread-state?${queryString}`;
      axios
        .get(url)
        .then((res) => {
  ...
  }, [uploadDate, threadDumpFiles,threadSummary, loadingSummary]);
    ...
    useEffect(() => {
      if (uploadDate && 
        ...
        const queryString = `uploadDate=${uploadDate}&` + threadDumpFiles.map(file => `threadDumpFile=${file}`).join('&');
        const url = `${apiBaseUrl}/api/threaddump/get-comparative-thread-pool?${queryString}`;
        axios
          .get(url)
          .then((res) => {
    ...
    }, [uploadDate, threadDumpFiles,threadSummary, loadingSummary, loadingStates]);
