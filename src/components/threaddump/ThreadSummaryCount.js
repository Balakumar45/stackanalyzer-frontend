...
const ThreadSummaryCount = ({ uploadDate, selectedFile,fileDetails, threadSummary, selectedState, setSelectedState }) => {
  ...
  const isLoadingSummary = !threadSummary || !selectedFile || !threadSummary[fileDetails[selectedFile]];

  // Define currentSummary and threadStates only if available
  const currentSummary = !isLoadingSummary ? threadSummary[fileDetails[selectedFile]] : {};
  ...
  const daemonPieData = {
    ...
    datasets: [
      {
        data: [
          (!isLoadingSummary && threadSummary[fileDetails[selectedFile]].daemonThreadCount) || 0, 
          (!isLoadingSummary && threadSummary[fileDetails[selectedFile]].nonDaemonThreadCount) || 0
        ],
  ...
  const fetchStackTraces = useCallback(async (state, pageNumber) => {
    ...
    const queryString = `uploadDate=${uploadDate}&` +
      selectedFile.map(file => `threadDumpFile=${file}`).join('&') +
      `&state=${state}&page=${pageNumber}`;
    const url = `${apiBaseUrl}/api/threaddump/get-state-traces?${queryString}`;
    try {
      ...
      {isLoadingSummary ? (
        ...
        <>
          ...
          <Paper sx={{ padding: 0.2, textAlign: "center", backgroundColor: "#f4f4f4", marginBottom: 1 }}>
            <Typography variant="h6">Total Threads: {threadSummary[fileDetails[selectedFile]].totalThreads}</Typography>
          </Paper>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              ...
              <Grid container spacing={0.5}>
                {[
                  { label: 'Daemon Threads', count: threadSummary[fileDetails[selectedFile]].daemonThreadCount, percentage: threadSummary[fileDetails[selectedFile]].daemonThreadPercentage },
                  { label: 'Non-Daemon Threads', count: threadSummary[fileDetails[selectedFile]].nonDaemonThreadCount, percentage: threadSummary[fileDetails[selectedFile]].nonDaemonThreadPercentage }
                ].map((item, index) => (
                  <Grid item xs={6} key={index}>
