...
import { Box, Container, Grid, Typography, Paper, TextField, Button, CircularProgress, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions } from '@mui/material';
...
const StackTraceLaunch = ({ setFileName }) => {
  ...
  const handleUpload = async (options = {}) => {
    ...
    const uploadOptions = { reuse: false, overwrite: false, ...options };
    ...
    try {
      ...
      if (response.status === 200) {
       ...
       navigate("/stacktrace/dashboard", { state: { fileName: response.data.fileName } });
      }
    } catch (error) {
...
export default StackTraceLaunch;